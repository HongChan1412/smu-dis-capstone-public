...
HTTPD_CONF="$APACHE_HOME/conf/httpd.conf"
...
if [ -f "$HTTPD_CONF" ]; then
    ...
    if grep -q "LimitRequestBody" $HTTPD_CONF; then
        echo "LimitRequestBody 설정이 이미 httpd.conf에 존재합니다."
        # 기존 설정을 5MB로 업데이트
        ...
        echo "</Directory>" | sudo tee -a $HTTPD_CONF > /dev/null
        echo "LimitRequestBody 설정을 $LIMIT_REQUEST_BODY_VALUE(5MB)로 httpd.conf에 추가했습니다."
    fi
    # Apache 서비스 재시작
    sudo systemctl restart httpd
    echo "Apache 서비스를 재시작하여 변경사항을 적용했습니다."
else
    echo "httpd.conf 파일을 찾을 수 없습니다. Apache 홈 디렉터리를 확인해 주세요."
fi
